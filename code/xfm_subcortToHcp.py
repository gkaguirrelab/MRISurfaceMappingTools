...
def xfm_subcortToHcp(recon_all_folder, segmentation_folder, subcortex_label, mni2mm, empty_cifti_cortex_left, empty_cifti_cortex_right, template_cifti, TR, workdir, outputdir):
    ...
    os.system('mri_convert %s %s' % (orig_file_mgz, orig_file_nii))
...
--convergence [100x70x50x20,1e-6,10] --shrink-factors 8x4x2x1 --smoothing-sigmas 3x2x1x0vox' % (workdir, workdir, workdir, mni2mm, orig_file_nii,
    ...
    os.system('mri_convert %s %s' % (segmentation_mgz, segmentation_nifti))
    
    segmentations_in_mni = os.path.join(workdir, 'MNI_ThalamicNuclei.v12.T1.FSvoxelSpace.nii.gz')
    os.system('antsApplyTransforms -d 3 -i %s -r %s -t %s -t %s -n NearestNeighbor -o %s' % (segmentation_nifti, mni2mm, 
                                                                                              warp, generic_affine, segmentations_in_mni))
    
        
    # Make the cifti label file
    os.system('wb_command -cifti-create-dense-from-template %s %s -series %s 0 -volume-all %s -metric CORTEX_LEFT %s -metric CORTEX_RIGHT %s' % (template_cifti, os.path.join(outputdir, 'subcortex_cifti.dtseries.nii'),
